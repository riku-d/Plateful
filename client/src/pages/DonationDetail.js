...
const DonationDetail = () => {
  ...
  useEffect(() => {
    const fetchDonation = async () => {
      try {
        ...
        // Updated to fetch from Food collection instead of Donation collection
        const res = await api.get(`/api/food/${id}`);
        console.log('Donation data received:', res.data); // Debug log
        
        // Handle the different data structure from Food API
        if (res.data.success && res.data.food) {
          // Map Food model fields to match what the component expects
          ...
          const mappedData = {
            ...
            donor: {
              name: foodData.donorName,
              phone: foodData.donorNumber,
              _id: foodData.donor || 'unknown'
            },
            expirationDate: foodData.expiryTime || new Date(),
            pickupDate: foodData.createdAt,
  ...
  const handleReserveDonation = async () => {
    ...
    try {
      setReserving(true);
      // Updated to use Food collection endpoint
      const res = await api.post(`/api/food/${id}/reserve`);
      if (res.data.success) {
  ...
  const handlePickupDonation = async () => {
    try {
      setReserving(true);
      // Updated to use Food collection endpoint
      const res = await api.post(`/api/food/${id}/pickup`);
      if (res.data.success) {
          ...
          <div className="border-t border-gray-200">
            <dl>
              ...
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm font-bold text-black sm:mt-0 sm:col-span-2">
