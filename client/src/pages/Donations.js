  ...
  useEffect(() => {
    const fetchDonations = async () => {
      try {
        ...
        // Ensure donor info exists and rating is consistent
        const donationsWithDonorInfo = data.map(donation => {
          if (!donation.donor) {
            return {
              ...donation,
              donor: { name: 'Unknown Donor', rating: { average: 0, count: 0 } }
            };
          }
          return {
            ...donation,
            donor: {
              ...donation.donor,
              rating: donation.donor.rating || { average: 0, count: 0 }
            }
          };
        });

        setDonations(donationsWithDonorInfo);
      } catch (err) {
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    {donation.donor?.rating?.average > 0 && (
                      <span className="text-sm text-gray-500 ml-2">
                        ({donation.donor.rating.average.toFixed(1)}★)
                      </span>
                    )}
                  </div>
                  <Link
