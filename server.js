...
// Function to mark expired food items
async function removeExpiredFood() {
  try {
    const now = new Date();
    const expiredFoods = await Food.find({
      expiryTime: { $lte: new Date(now.getTime() + (10 * 60 * 1000)) },
      status: { $nin: ['expired', 'delivered', 'cancelled'] } // Only update items that aren't already expired, delivered or cancelled
    });
    
    if (expiredFoods.length > 0) {
      console.log(`🕒 Marking ${expiredFoods.length} food donations as expired`);
    ...
    console.error('❌ Error marking expired food:', err);
  }
}
